...
func Eventdetailhome(c *fiber.Ctx) error {
	type payload_eventdetail struct {
	...
	bearToken := c.Get("Authorization")
	token := strings.Split(bearToken, " ")
	client := new(payload_eventdetail)
	if err := c.BodyParser(client); err != nil {
		c.Status(fiber.StatusBadRequest)
		return c.JSON(fiber.Map{
			"status":  fiber.StatusBadRequest,
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"event_idmemberagen": client.Event_idmemberagen,
		}).
		Post(PATH + "api/eventdetail")
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Response Info:")
...
func Eventdetailmemberhome(c *fiber.Ctx) error {
	type payload_eventdetail struct {
		Page     string `json:"page"`
		Sdata    string `json:"sdata" `
		Event_id int    `json:"event_id" `
	...
	resp, err := axios.R().
		...
		Post(PATH + "api/eventgroupdetail")
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Response Info:")
...
func EventDetailSave(c *fiber.Ctx) error {
	type payload_eventdetailsave struct {
		Page                     string `json:"page"`
		Sdata                    string `json:"sdata" `
	...
	bearToken := c.Get("Authorization")
	token := strings.Split(bearToken, " ")
	client := new(payload_eventdetailsave)
	if err := c.BodyParser(client); err != nil {
		c.Status(fiber.StatusBadRequest)
		return c.JSON(fiber.Map{
			"status":  fiber.StatusBadRequest,
...
func EventDetailSaveStatus(c *fiber.Ctx) error {
	type payload_eventdetailsavestatus struct {
		Page                string `json:"page"`
		Sdata               string `json:"sdata" `
		Eventdetail_id      int    `json:"eventdetail_id" `
		Eventdetail_idevent int    `json:"eventdetail_idevent" `
	...
	bearToken := c.Get("Authorization")
	token := strings.Split(bearToken, " ")
	client := new(payload_eventdetailsavestatus)
	if err := c.BodyParser(client); err != nil {
		c.Status(fiber.StatusBadRequest)
		return c.JSON(fiber.Map{
			"status":  fiber.StatusBadRequest,
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"eventdetail_id":      client.Eventdetail_id,
			"eventdetail_idevent": client.Eventdetail_idevent,
			"eventdetail_status":  client.Eventdetail_status,
		}).
		Post(PATH + "api/eventdetailstatusupdate")
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Response Info:")
