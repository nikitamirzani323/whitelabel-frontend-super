<script>
  ...
  $: {
    if (searchMovie) {
      filterMovie = listHome.filter(
        (item) =>
          item.movie_status.toLowerCase().includes(searchMovie.toLowerCase()) ||
          item.movie_title.toLowerCase().includes(searchMovie.toLowerCase()) ||
          item.movie_year.toLowerCase().includes(searchMovie.toLowerCase())
      );
    } else {
      filterMovie = [...listHome];
    }
    if (searchMovieMini) {
      filterMovieMini = listmoviemini.filter((item) =>
        item.movie_title.toLowerCase().includes(searchMovieMini.toLowerCase())
      );
    } else {
  ...
  async function call_genre() {
    ...
    if (json.status == 200) {
      ...
      if (record != null) {
        let totalrecordcategory = record.length;
        let no = 0;
        for (var i = 0; i < record.length; i++) {
          no = no + 1;
          listgenre = [
  ...
  async function handleDeleteMovieSource(e) {
    let temp = movie_field_source.filter(
  ...
  async function handleDeleteMovieGenre(e) {
    let temp = movie_field_genre.filter(
      (item) => item.movie_genre_id !== parseInt(e)
    );
    movie_field_genre = [];
    for (var i = 0; i < temp.length; i++) {
      movie_field_genre = [
    ...
    <div class="row">
      <div class="col-sm-6">
        ...
        <div class="mb-3">
          ...
          <select bind:value={movie_field_status} class="form-control required">
            ...
            <tbody>
              {#each movie_field_genre as rec}
                <tr>
                  <td
                    width="1%"
                    style="cursor: pointer;text-align:center;vertical-align:top;"
                  >
                    <i
                      on:click={() => {
              ...
              {#each movie_field_source as rec}
                <tr>
                  ...
                  <td
                    ...
                    width="*"
                    style="text-align:left;vertical-align:top;font-size:12px;cursor:pointer;text-decoration:underline;color:blue;"
                  >
          ...
          <tr>
            ...
            {#if album_flagclick == true}
              <td
                ...
                NOWRAP
                style="text-align: left;vertical-align: top;font-size: {table_body_font};{album_css}"
                >{rec.album_filename}</td
              >
            {:else}
              <td
                NOWRAP
          ...
          <tr>
            ...
            {#if genre_flagclick == true}
              <td
                ...
                NOWRAP
                style="text-align: left;vertical-align: top;font-size: {table_body_font};{genre_css}"
                >{rec.genre_name}</td
              >
            {:else}
              <td
        ...
        {#each listslide as rec}
          <tr>
            ...
            <td
              NOWRAP
              style="text-align: left;vertical-align: top;font-size: {table_body_font};{genre_css}"
              >{rec.slide_movietitle}</td
            >
            <td
              NOWRAP
              style="text-align: right;vertical-align: top;font-size: {table_body_font};"
              >{rec.slide_position}</td
            >
          </tr>
        {/each}
      </tbody>
